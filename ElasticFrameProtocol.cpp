...
ElasticFrameProtocol::unpackType1(const uint8_t *pSubPacket, size_t packetSize, uint8_t fromSource) {
    ...
    // be triggered by now.
    // I invalidate this bucket to save me but the user should be notified somehow about this state.
...
ElasticFrameMessages ElasticFrameProtocol::addEmbeddedData(std::vector<uint8_t> *pPacket,
    ...
    if (isLast) lEmbeddedHeader.embeddedFrameType |= ElasticEmbeddedFrameContent::lastembeddedcontent;
